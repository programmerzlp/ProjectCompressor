...
public class CustomCssCompressor {

	private StringBuffer srcsb = new StringBuffer();

	public CustomCssCompressor(Reader in) throws Exception {
		// Read the stream...
		int c;
		while ((c = in.read()) != -1) {
			srcsb.append((char) c);
	...
	protected String extractDataUrls(String css, ArrayList<String> preservedTokens) {
		 ...
		 * ' and ) being part of the data string. Hence switching to indexOf, to
		 * determine whether or not we have matching string terminators and
		...
		while (m.find()) {
			...
			if (foundTerminator) {
				...
				String preserver = "url(___YUICSSMIN_PRESERVED_TOKEN_"
	...
	private String preserveOldIESpecificMatrixDefinition(String css,
			ArrayList<String> preservedTokens) {
		StringBuffer sb = new StringBuffer();
		Pattern p = Pattern
				.compile("\\s*filter:\\s*progid:DXImageTransform.Microsoft.Matrix\\(([^\\)]+)\\);");
		Matcher m = p.matcher(css);
		while (m.find()) {
			String token = m.group(1);
			preservedTokens.add(token);
			String preserver = "___YUICSSMIN_PRESERVED_TOKEN_" + (preservedTokens.size() - 1)
					+ "___";
	...
	public void compress(Writer out, int linebreakpos) throws IOException {
		...
		while ((startIndex = sb.indexOf("/*", startIndex)) >= 0) {
			...
			comments.add(token);
			sb.replace(startIndex + 2, endIndex, "___YUICSSMIN_PRESERVE_CANDIDATE_COMMENT_"
					+ (comments.size() - 1) + "___");
			startIndex += 2;
		}
		css = sb.toString();
		...
		// If there are multiple @charset directives, push them to the top of
		// the file.
		sb = new StringBuffer();
		...
		// When all @charset are at the top, remove the second and after (as
		...
		sb = new StringBuffer();
		p = Pattern
				.compile("(?i):(active|after|before|checked|disabled|empty|enabled|first-(?:child|of-type)|focus|hover|last-(?:child|of-type)|link|only-(?:child|of-type)|root|:selection|target|visited)");
		m = p.matcher(css);
		while (m.find()) {
			m.appendReplacement(sb, ':' + m.group(1).toLowerCase());
		}
		...
		sb = new StringBuffer();
		p = Pattern
				.compile("(?i):(lang|not|nth-child|nth-last-child|nth-last-of-type|nth-of-type|(?:-(?:moz|webkit)-)?any)\\(");
		m = p.matcher(css);
		while (m.find()) {
			m.appendReplacement(sb, ':' + m.group(1).toLowerCase() + '(');
		}
		...
		// and() is already done for us right after this
		sb = new StringBuffer();
		p = Pattern
				.compile("(?i)([:,\\( ]\\s*)(attr|color-stop|from|rgba|to|url|(?:-(?:atsc|khtml|moz|ms|o|wap|webkit)-)?(?:calc|max|min|(?:repeating-)?(?:linear|radial)-gradient)|-webkit-gradient)");
		m = p.matcher(css);
		while (m.find()) {
		...
		css = css.replaceAll(
				"(?i)(^|[^0-9])(?:0?\\.)?0(?:px|em|%|in|cm|mm|pc|pt|ex|deg|g?rad|m?s|k?hz)", "$10");
		...
		sb = new StringBuffer();
		p = Pattern
				.compile("(?i)(background-position|webkit-mask-position|transform-origin|webkit-transform-origin|moz-transform-origin|o-transform-origin|ms-transform-origin):0(;|})");
		m = p.matcher(css);
		while (m.find()) {
		...
		while (m.find()) {
			...
			for (i = 0; i < rgbcolors.length; i++) {
				...
				// Probably should throw out a warning here, but generating
				// valid CSS is a bigger concern.
				if (val > 255) {
					val = 255;
				}
		...
		// We also want to make sure we're only compressing #AABBCC patterns
		// inside { }, not id selectors ( #FAABAC {} )
		// We also want to avoid compressing invalid values (e.g. #AABBCCD to
		// #ABCD)
		p = Pattern.compile("(\\=\\s*?[\"']?)?"
		...
		while (m.find(index)) {
			...
			if (isFilter) {
				...
				sb.append(m.group(1) + "#" + m.group(2) + m.group(3) + m.group(4) + m.group(5)
						+ m.group(6) + m.group(7));
			} else {
				if (m.group(2).equalsIgnoreCase(m.group(3))
					...
					sb.append("#"
							+ (m.group(2) + m.group(3) + m.group(4) + m.group(5) + m.group(6) + m
									.group(7)).toLowerCase());
				}
			}
		...
		css = css.replaceAll("\\(([\\-A-Za-z]+):([0-9]+)\\/([0-9]+)\\)",
		...
		// TODO: Should this be after we re-insert tokens. These could alter the
